...
local function java_keymaps()
    ...
    vim.cmd("command! -buffer SayHello echo 'hello'")

    -- Set a Vim motion to <Space> + <Shift>J + o to organize imports in normal mode
    vim.keymap.set('n', '<leader>Jo', "<Cmd> lua require('jdtls').organize_imports()<CR>", { desc = "[J]ava [O]rganize Imports" }) -- Set a Vim motion to <Space> + <Shift>J + v to extract the code under the cursor to a variable
...
local function setup_jdtls()
    ...
    local settings = {
        java = {
            ...
            completion = {
                -- When using an unimported static method, how should the  rank possible places to import the static method from
    ...
    local on_attach = function(_, bufnr)
        ...
        --vim.api.nvim_create_autocmd("BufWritePost", {
          --  pattern = { "*.java" },
            --callback = function()
            --    local _, _ = pcall(vim.codelens.refresh)
            --end
        --})
    end
